...
test("server advertises zoron_health", async () => {
  const { client, close } = await connectTestClient();
  try {
    assert.equal(client.getServerVersion().name, "zoron");
    const { tools } = await client.listTools();
    const names = tools.map((t) => t.name);
    assert.ok(names.includes("zoron_health"), `expected zoron_health in ${names}`);
  } finally {
    await close();
  }
});

test("zoron_health reports pipeline import and configuration", async () => {
  const { client, close } = await connectTestClient({
    GEMINI_API_KEY: "sk-test-key",
    SKIP_HEAVY_SEARCH: "true",
  });
  try {
    const result = await client.callTool({ name: "zoron_health", arguments: {} });
    ...
    assert.match(result.content[0].text, /Zoron MCP server ready/);
  } finally {
    await close();
  }
});

test("zoron_health flags a missing Gemini key", async () => {
